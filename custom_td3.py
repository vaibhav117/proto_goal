...
class TD3(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100, action_l2=0.2) -> None:
        wandb.config.action_l2 = action_l2
        ...
        for _ in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                next_actions = (self.actor(replay_data.next_observations) + noise) # TODO use only actor without target network
            ...
            if self._n_updates % self.policy_delay == 0:
                # Compute actor loss
                # with torch.no_grad()
                actions_real = self.actor(replay_data.observations)
                ...
                # print(self.critic.parameters().__iter__() ,  "critic parameters")
        ...
        if len(actor_losses) > 0:
            self.logger.record("train/actor_loss", np.mean(actor_losses).item())
            
        self.logger.record("train/critic_loss", np.mean(critic_losses).item())
        # print(np.mean(actor_losses).item())
        wandb.log({"train/n_updates": self._n_updates, "train/actor_loss":np.mean(actor_losses).item(),"train/critic_loss":np.mean(critic_losses).item()})
    def learn(
        self,
        total_timesteps: int,
